...
const FundsPage = () => {
    ...
    const [showWithdrawalSuccessMessage, setShowWithdrawalSuccessMessage] = useState(false);
    const [showWithdrawalErrorMessage, setShowWithdrawalErrorMessage] = useState(false);
    const [showAccountCreatedMessage, setShowAccountCreatedMessage] = useState(false);
    const [showAccountNotCreatedMessage, setShowAccountNotCreatedMessage] = useState(false);
        ...
        <div>
            ...
            <div>
                {chargesEnabled ? (
                    <div>
                        <p>Withdraw funds:</p>
                        <p><i>We charge a 4% service fee with every withdrawal to keep the service running. Thank you for playing!</i></p>
                        <input
                            type="number"
                            min="0"
                            max={balance}
                        ...
                        />
                        <button onClick={handleWithdrawBalance} disabled={!validateWithdrawalAmount()}>
                            Withdraw
                        </button>
                    </div>
                ) : (
            ...
            {showWithdrawalSuccessMessage && <p>Funds withdrawn successfully! Your balance should update soon.</p>}
            {showWithdrawalErrorMessage && <p>Funds not withdrawn. Please try again.</p>}
            {showAccountCreatedMessage && <p>Stripe connected account created successfully!</p>}
            {showAccountNotCreatedMessage && <p>Stripe connected account not created. Please try again.</p>}
        </div>
    );
};
